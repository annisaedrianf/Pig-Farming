...
namespace cetho.Module.E2E.Tests;

public class cethoTests : IDisposable {
    const string BlazorAppName = "cethoBlazor";
    const string WinAppName = "cethoWin";
    const string AppDBName = "cetho";
    EasyTestFixtureContext FixtureContext { get; } = new EasyTestFixtureContext();

	public cethoTests() {
        FixtureContext.RegisterApplications(
            new BlazorApplicationOptions(BlazorAppName, string.Format(@"{0}\..\..\..\..\cetho.Blazor.Server", Environment.CurrentDirectory)),
            new WinApplicationOptions(WinAppName, string.Format(@"{0}\..\..\..\..\cetho.Win\bin\EasyTest\net8.0-windows\cetho.Win.exe", Environment.CurrentDirectory))
        );
        FixtureContext.RegisterDatabases(new DatabaseOptions(AppDBName, "cethoEasyTest", server: @"(localdb)\mssqllocaldb"));	           
	}
    public void Dispose() {
